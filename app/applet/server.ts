...
app.post("/api/send-report", upload.single("pdf"), async (req, res) => {
  try {
    ...
    const emailList = JSON.parse(emails); // Expecting JSON array string

    // In a real scenario, you'd use a real SMTP server. 
    // Here we check for credentials, or use ethereal for testing if none provided.
    let transporter;
    if (process.env.SMTP_HOST && process.env.SMTP_USER && process.env.SMTP_PASS) {
      ...
      // Create a test account if no SMTP provided
    ...
    // If using ethereal, you can get the preview URL
